...
func handle_controller(_input_dir: Vector2, can_fly: bool = false) -> void:
	
	if _input_dir:
		if can_fly:
			direction = player.head.camera.global_transform.basis * Vector3(_input_dir.x, 0, _input_dir.y)
		else:
			direction = player.head.transform.basis * Vector3(_input_dir.x, 0, _input_dir.y)
